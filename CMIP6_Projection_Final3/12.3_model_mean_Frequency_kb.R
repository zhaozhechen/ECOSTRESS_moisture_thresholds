...
# From 15 models
# In each model, the frequency was calculated using different k and b combinations
# Which are the upper and lower k and b combinations
...
Freq_path <- "/fs/ess/PAS2204/Results/CONUS_Threshold_Final/CMIP6_Projection_Final3/Frequency_stacks_kb/"
...
out <- stack()
for(kb in 1:2){
  # Initialize a stack to store all rasters from 15 models, for the current kb combination
  freq_stack <- stack()
  for(model_name in models_ls){
    freq <- readRDS(paste0(Freq_path,model_name,"_",time_name,"_",ssp,"_freq_kb",kb,".rds"))
...
names(out) <- c("Freq_kb1","Freq_kb2")
saveRDS(out,paste0(Freq_path,"Model_mean_",time_name,"_",ssp,"_freq_kb.rds"))
