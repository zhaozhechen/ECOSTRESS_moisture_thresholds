...
# This code is to get upper and lower boundaries of frequency
# When only change SM or only change VPD
# So, two combinations of kb
# For two scenarios: use CMIP6 SM and historical VPD
# Or use CMIP VPD and historical SM
# Reference code: /fs/ess/PAS2204/Code/CONUS_Threshold/CMIP6_Projection_Final3/5.4_Joint_Frequency_SM_VPD.R
...
Output_path <- "/fs/ess/PAS2204/Results/CONUS_Threshold_Final/CMIP6_Projection_Final3/Frequency_stacks_SM_VPD_kb/"
...
# ID 1-4 for four jobs
ID <- as.numeric(substr(arrayid,1,1))
if(ID == 1){
  # Determine which set of k and b to use
  # If kb_chg = 1, Use alpha + sd(alpha) and the corresponding b
  # If kb_chg = 2, Use alpha - sd(alpha) and the corresponding b
  kb_chg <- 1
  # Determine to change SM or VPD
...
# This is the combination of VPD and SM to be used to calculate frequency
...
saveRDS(freq,paste0(Output_path,model_name,"_",time_name,"_",
                    ssp,"_",out_chg_name,"_freq_kb",kb_chg,".rds"))
