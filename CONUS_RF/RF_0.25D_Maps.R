...
var_ls <- c("Theta","SM_50_abs","VPD_50_abs","LC")
...
for(i in 1:length(var_ls)){
  ...
  if(var_name == "Theta"){
    g <- g + 
      ...
      labs(title=expression("Metric"~alpha~(degree)))
    ...
    g <- g + 
      scale_fill_gradientn(na.value="white",
                           colours = BrBG_color,
                           guide = guide_colorbar(frame.colour = "black",ticks.colour = "black"))+
      labs(title = bquote("Metric"~SM[50]~(m^3/m^3)))
    ...
    g <- g + 
      scale_fill_gradient(na.value="white",
                          ...
                          high="#FF0110",
                          guide = guide_colorbar(frame.colour = "black",ticks.colour = "black"))+
      labs(title = bquote("Metric"~VPD[50]~" (kPa)"))
  }
...
pdf(paste0(Output_path,"0.25D_Maps.pdf"),
    height=6,
    width=12)
print(g)
dev.off()
