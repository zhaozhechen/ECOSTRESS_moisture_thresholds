...
df$Type[df$AI < 0.05] <- "1"
df$Type[df$AI >= 0.05 & df$AI < 0.2] <- "2"
df$Type[df$AI >= 0.2 & df$AI < 0.5] <- "3"
df$Type[df$AI >= 0.5 & df$AI < 0.65] <- "4"
df$Type[df$AI >= 0.65] <- "5"
...
for(i in 1:3){
  ...
  g <- ggplot(data=df,aes(x=Dry,y=.data[[var_name]],fill = Dry))+
    geom_boxplot(outlier.size=0.2,fill=my_color)+
    geom_signif(
      comparisons = list(c("Arid","Humid")),
...
pdf(paste0(Output_path,"0.25D_210m_agg_boxplots_small.pdf"),
    height=4,
    width=8)
print(g)
dev.off()
