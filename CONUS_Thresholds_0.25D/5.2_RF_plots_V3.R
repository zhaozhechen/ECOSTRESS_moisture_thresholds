...
#Input_path <- "/fs/ess/PAS2204/Results/CONUS_Threshold_0.25D/RF_210m_aggregated/IM_files_V3/"
Input_path <- "/fs/ess/PAS2204/Results/CONUS_Threshold_0.25D/RF_210m_aggregated/IM_files_ALE/"

# Output path of RF figures
#Output_path <- "/fs/ess/PAS2204/Results/CONUS_Threshold_0.25D/Figures/"
#Output_path <- "/fs/ess/PAS2204/Results/CONUS_Threshold_0.25D/Figures/RF_V3/"
Output_path <- "/fs/ess/PAS2204/Results/CONUS_Threshold_0.25D/Figures/RF_ALE/"
############
# Functions
############
my_theme <- theme(
...
IP_df <- IP_df %>% 
  mutate(Group = replace(Group,var_ls == "T_gs_mean"|var_ls == "p_gs_mean"|
                           var_ls == "vpd_gs_mean"|var_ls == "SM_gs_mean","Long-term climate"),
         Group = replace(Group,var_ls == "CH"|var_ls == "RD"|var_ls == "LC"|var_ls == "LC_shannon","Ecological condition"),
         Group = replace(Group,var_ls == "Ks"|var_ls == "T_Sand"|var_ls == "n_parameter","Soil property"),
         #Group = replace(Group,var_ls == "g1"|var_ls == "gpmax"|var_ls == "C"|var_ls == "P50","Plant hydraulic trait"),
         Group = replace(Group,var_ls == "DEM"|var_ls == "DEM_sd","Topography"))
# Rename variables
IP_df <- IP_df %>%
  mutate(var_ls = replace(var_ls,var_ls == "T_gs_mean","Tair"),
         ...
         var_ls = replace(var_ls,var_ls == "vpd_gs_mean","VPD"),
         var_ls = replace(var_ls,var_ls == "p_gs_mean","P"),
         var_ls = replace(var_ls,var_ls == "CH","Canopy height"),
         var_ls = replace(var_ls,var_ls == "DEM_sd","Topographic diversity"),
         var_ls = replace(var_ls,var_ls == "LC_shannon","LC diversity"),
         var_ls = replace(var_ls,var_ls == "LC","LC"),
         var_ls = replace(var_ls,var_ls == "SM_gs_mean","SM"),
...
# Get the first 9 most important variable names
var_ls <- rownames(IP_df)[order(IP_df$IP_mean,decreasing = TRUE)][1:9]
...
pdf(paste0(Output_path,"0.25D_210m_agg_RF_Plots_ALE.pdf"),
    height=6,
    width=14)
print(g_all)
dev.off()
...
# ------------------
# # Get the least 3 most important variable names
var_ls <- rownames(IP_df)[order(IP_df$IP_mean,decreasing = TRUE)][11:13]
...
# Combine all plots
g_all_least <- plot_grid(plotlist=pdp_g_ls,ncol=3)
pdf(paste0(Output_path,"0.25D_210m_agg_additional_RF_ALE_Plots.pdf"),
    height=2,
    width=7)
print(g_all_least)
dev.off()
