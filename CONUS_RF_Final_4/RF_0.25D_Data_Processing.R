...
# This code is to get RF variables at 0.25D
# Add soil hydraulic traits, diversity variables, and aridity index
# Output full rasters (no theta or uncertainty mask)
# Reference code: /fs/ess/PAS2204/Code/CONUS_Threshold/CONUS_RF_Final_3/RF_0.25D_Data_processing_20240212.R
...
feature_path <- "/fs/ess/PAS2204/Results/CONUS_Threshold_Final/RF_Samples/0.25degree_all/"
...
# Output path
Output_path <- "/fs/ess/PAS2204/Results/CONUS_Threshold_Final/CONUS_RF_Final_4/0.25D_rasters_all/"
...
raster_stack <- readRDS(paste(feature_path,"Var_stack_0.25degree_Block_",Block_ID,".rds",sep=""))
...
# Loop over each cell in this raster_stack and extract required variables
for(i in 1:ncell(raster_stack)){
  # Extent of this cell
  ...
  print(paste("Complete",round(i/ncell(raster_stack),4)*100,"%"))
}
