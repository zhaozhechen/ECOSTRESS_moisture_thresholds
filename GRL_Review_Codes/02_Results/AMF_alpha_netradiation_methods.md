...
10. Preserve the original uncertainty routine: 100 bootstrap resamples of the boundary points, seed=1, with the original 1.5-IQR outlier removal before taking SD. This is not a bootstrap of independent observation datasets or a statistical test of the difference between the nested datasets. At the user's request, the figure shows point estimates only. The bootstrap SD and explicit lower and upper one-SD bounds remain in the summary table. These are descriptive SD summaries, not 95% confidence intervals; the US-CF3 upper bound exceeds 90 degrees because the original linear SD is large and should not be interpreted as a feasible alpha interval.
