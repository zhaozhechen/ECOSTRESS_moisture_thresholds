...
delta_freq_SM_VPD_map <- g_delta_map(delta_freq_SM_VPD,p_value_SM_VPD,color_delta_freq,bquote("SM and VPD-Induced"~Delta~"Frequency"))
delta_freq_SM_map <- g_delta_map(delta_freq_SM,p_value_SM,color_delta_freq,bquote("SM-Induced"~Delta~"Frequency"))
delta_freq_VPD_map <- g_delta_map(delta_freq_VPD,p_value_VPD,color_delta_freq,bquote("VPD-Induced"~Delta~"Frequency"))
delta_freq_compound_map <- g_delta_map(delta_freq_compound,p_value_compound,color_delta_freq,"Compound effect of SM and VPD")

# Combine these 4 maps
g <- plot_grid(delta_freq_VPD_map,delta_freq_SM_map,
               delta_freq_SM_VPD_map,delta_freq_compound_map,
               nrow = 2,
               labels = c("c","d","e","f"))
...
# Make map for historical frequency ---------------------
df_hist <- as.data.frame(Hist_freq,xy=TRUE)
g_hist <- ggplot()+
  geom_tile(data = df_hist,aes(x=x,y=y,fill=Freq))+
  geom_sf(data=CONUS,fill=NA,color="black",alpha=0.5)+
  my_theme+
  scale_fill_gradientn(na.value="white",
                       colours = color_freq,
...
pdf(paste0(Output_path,"Frequency_maps_hist.pdf"),
    height=4,
    width=7.5)
print(g_hist)
dev.off()
